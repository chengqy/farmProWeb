...
public class UserController {
	...
	public String login(UserParam userParam, HttpSession session, Model model) {
		 ...
		 * bindingResult.getAllErrors(); model.addAttribute("errors",errors);
		 * return "login"; }
		 */
		String message = null;
		boolean flag = true;
	...
	public String updatePassword(HttpSession session, UserValidatorParam userValidatorParam, 
			Model model,String oldPassword) throws Exception {
		String message = null;
	...
	public String bindphone(HttpSession session, String code, String code2,String phone,
		...
		// 验证验证码
		}else if(!code.equals(session.getAttribute(user.getPhone()))||!code2.equals(session.getAttribute(phone))){
			message = "验证码错误";
			flag=false;
		}
		if(flag){
	 ...
	 * 发送换绑手机验证码
	 */
	@RequestMapping(value = "/sendBindMessage", method = { RequestMethod.GET })
	public String sendBindMessage(HttpSession session, String phone, Model model)
			throws Exception {
		//验证手机
		String message = null;
		if (phone.equals("null") || phone == null || phone.equals("")) {
	 ...
	 * 发送换绑旧手机验证码
	 */
	@RequestMapping(value = "/sendBindOldMessage", method = { RequestMethod.GET })
	public String sendBindOldMessage(HttpSession session, String phone, Model model)
			throws Exception {
		String message = null;
		...
		if (!phone.equals(user.getPhone())){
			throw new MyException("号码有误！");
		}
		
		//获取随机验证码
		String code = SmsUtils.getCode();
		//发送短信
		SendSmsResponse smsResponse = SmsUtils.sendSms(user.getPhone(), code);
		//发送成功
		if (smsResponse.getCode() != null && smsResponse.getCode().equals("OK")) {
			message = "验证码发送成功！请5分钟内填写";
			//保存号码为session，值为验证码
